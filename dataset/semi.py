...
class SemiDataset(Dataset):
    def __init__(self, name, root, mode, size, labeled_id_path=None, unlabeled_id_path=None, pseudo_mask_path=None, transform=None, s_transform=None, normalize=None, timecode=None):
        """
    ...
    def __getitem__(self, item):
        id = self.ids[item]
        if 'multiyear' in self.name:
            image_path = os.path.join(self.root, 'image', id + '.png')
        else:
            image_path = os.path.join(self.root, 'image', id + '.png')
        ...
        if self.timecode != None:
            domain = torch.zeros(len(self.timecode))
            time = id.split("(")[-1].split(")")[0]
            domain[self.timecode.index(time)] = 1
            return img.to(torch.float), img_w.to(torch.float), mask.squeeze().to(torch.float), domain.to(torch.float)
