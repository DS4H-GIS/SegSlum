...
import model.backbone.resnet_whitening as resnet_whitening
...
class DeepLabV3Plus(BaseNet):
    ...
    def base_forward(self, x, training=True):
        h, w = x.shape[-2:]

        c1, _, _, c4 = self.backbone.base_forward(x, training)
...
class DeepLabV3Plus_Aux(nn.Module):
    def __init__(self, backbone, dilations, nclass):
        ...
        if 'whitening' in backbone:
            self.backbone = resnet_whitening.__dict__[backbone](pretrained=True, replace_stride_with_dilation= [False, False, True])
...
#         self.classifier_domain = #feature를 1차원 벡터로 만들어서 reverse gradient layer를 붙여서 domain classifier를 붙인다.

        self.classifier = nn.Conv2d(256, nclass, 1, bias=True)
        self.classifier_g = nn.Linear(256, 2)

    def forward(self, x, train=True, need_fp=False):
        h, w = x.shape[-2:]
...
class DeepLabV3Plus_Aux_RGL(nn.Module):
    def __init__(self, backbone, dilations, nclass):
        super(DeepLabV3Plus_Aux_RGL, self).__init__()
        ...
        self.discriminator = nn.Sequential(nn.Linear(256 * 64 * 64, 2048),
                                           nn.BatchNorm1d(2048),
                                           ...
                                           nn.ReLU(inplace=True),
                                           nn.Linear(1024, 3))
        #feature를 1차원 벡터로 만들어서 reverse gradient layer를 붙여서 domain classifier를 붙인다.(GRL 후에 FC layer)

        self.classifier = nn.Conv2d(256, nclass, 1, bias=True)
        self.classifier_g = nn.Linear(256, 2)

    def forward(self, x, need_fp=False, alpha=4):
        h, w = x.shape[-2:]
        ...
        if need_fp:
            ...
            # [8, 256, 64, 64]
            reverse_feature = feature.view(-1, 256 * 64 * 64)
            reverse_feature = ReverseLayerF.apply(reverse_feature, alpha)
            domain_out = self.discriminator(reverse_feature)

            return out, out_fp, gram_feature, gram_feature_fp, domain_out
        ...
        return out, gram_feature, domain_out
